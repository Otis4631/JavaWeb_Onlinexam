...
public class TestDao implements ITestDao {
	...
	public List<Map<String, Object>> getTestByStudent(int id, String currData) {
		String sql = "SELECT t.question_ids,t.course_id, t.id,cs.`name` as courseName,t.name as testName, "
				...
				+ " class c,student s WHERE s.id=? AND find_in_set(s.class_id,t.class_ids) AND s.class_id=c.id  AND t.course_id=cs.id AND t.end_time > ? AND t.id not in(select test_id from papers where student_id = ?)";
		try {
			return dbUtil.getQueryList(sql, new Object[] {id,currData,id});
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return new ArrayList<>();
